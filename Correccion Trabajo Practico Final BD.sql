...
INSERT INTO facturas (letra, numero, fecha, monto, id_cliente, legajo_vendedor) VALUES
    ...
    --  1 cantidad de clientes y vendedores que tiene el negocio   MAL

select (select count(id_cliente) 'cantidad_clientes' from clientes),
	   (select count(legajo) 'cantidad_vendedores' from vendedores); -- corregido
    
	-- 2 Listar los clientes que tienen un email registrado, ordenado por apellido y nombre  MAL
    
    select		apellido, nombre
    from		clientes
    where		email is not null and email not in(' ')
    order by	apellido, nombre; -- corregido
    
    -- 3 Listar nombre y apellido, tanto de clientes como de vendedores, cuya direccion de email sea de Gmail  MAL

    select	concat(nombre, ' ', apellido) 'nombre y apellido', 'cliente' as 'cliente/vendedor' from clientes where email like '%gmail.com%'
    union
    select	concat(nombre, ' ', apellido), 'vendedor' from vendedores where email like '%@gmail.com%'; -- corregido
...
-- 6 listar los articulos cuyo nombre esté compuesto por mas de dos palabras   MAL

select		producto
from		articulos
where		producto like '% % %';  -- corregido
...
-- 9 informar la cantidad de facturas de cada cliente  MAL

select		c.id_cliente, c.nombre, count(letra) as 'cantidad de facturas'
from		facturas f
right join	clientes c
on 			c.id_cliente=f.id_cliente
group by	c.id_cliente, nombre; -- corregido
...
-- 14 informar cuantas unidades se vendieron de cada articulo -- MAL


select		a.producto, sum(v.cantidad) 'cantidad de unidades vendidas'
from		articulos a
join	  	ventas v
on			v.codigo=a.codigo
group by	a.producto; -- corregido
...
-- 19 eliminar los productos que no se hayan vendido nunca   MAL

delete 		a
from		articulos a
left join	ventas v
on 			a.codigo=v.codigo
where		v.codigo is null; -- corregido
